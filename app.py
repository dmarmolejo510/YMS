...
from YMS.Docks import Inicio as TMS_Dock
...
@app.route("/YMS/Docks",methods=['GET','POST'])
def YMS_Docks():
    if "IDu" not in session:
        return render_template_string("<script>window.location= '"+str(request.url_root)+"';</script>")
    else:
        if request.method == "GET":
            return render_template_string(TMS_Dock.Inicio())
        else:
            Datos = {}
            for K in request.form.keys():
                Datos[K] = escape(request.form[K]).striptags()
            return render_template_string(TMS_Dock.Direccionar(Datos))

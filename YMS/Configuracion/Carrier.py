            ...
            <div class='col-12'>
                ...
                <script>
                    ...
                    var Tabla_Proveedores = new Tabulator("#Tabla_Proveedores", {
                        ...
                        data:"""+str(Patio)+""",
                        columns:[
                            {field:"Nombre","title":"Nombre",widthGrow:1,headerFilter:"input"},
...
def Nuevo_Destino(Datos):
    ...
    try:
        ...
        Formulario["Campos"].append({"tipo":"texto","campo":"Codigo","titulo":"Nombre","Requerido":1,"min":1,"max":30,"valor":""})
...
def Nuevo_Destino_Guardar(Datos):
    ...
    try:
        ...
        Existe = DB.Get_Dato("SELECT * FROM "+str(BD_Nombre)+".ccarrier WHERE cca_nombre = '"+str(Info_Datos["Codigo"])+"' and cca_activo = '1' ")
        if len(Existe) == 0:
            Error = DB.Instruccion("""
            INSERT INTO """+str(BD_Nombre)+""".ccarrier
            (cca_nombre)
            VALUES
            ('"""+str(Info_Datos["Codigo"])+"""')
            """)
            if Error == "":
                Resultado["Estado"] = 1
            else:
...
def Eliminar(Datos):
    ...
    Error = ""
    try:
        Error = DB.Instruccion("UPDATE "+str(BD_Nombre)+".ccarrier SET cca_activo = 0 WHERE cca_nombre = '"+str(Datos["ID"])+"'")
        if Error == "":
            Resultado["Estado"] = 1
        else:
            Resultado["Contenido"] += str(Error)

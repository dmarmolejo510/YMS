...
def Inicio():
    ...
    try:
        ...
        """
        Cajas_Aqui = []
        Programacion_Abiertos = DB.Get_Dato("SELECT * FROM "+str(BD_Nombre)+".ccajas WHERE (cc_activo = 1 or cc_informacion_actual LIKE concat('%',DATE(NOW()),'%') or DATE(cc_fecha_hora) = DATE(NOW()) ) AND cc_tipo_actual = 'Outbound' AND cc_informacion_actual LIKE '%Fecha_Salida%'")
